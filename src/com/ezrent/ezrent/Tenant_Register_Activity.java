...
public class Tenant_Register_Activity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		tenant_phone_number = (EditText) findViewById(R.id.tenant_phone_number_EditText);
	...
	public OnClickListener cancelButtonListener = new OnClickListener()
	{
		...
		public void onClick(View arg0)
		{
			...
			Intent intent = new Intent(Tenant_Register_Activity.this, LogIn.class);
	...
	public OnClickListener tenant_registerButtonListener = new OnClickListener()
	{
		...
		public void onClick(View arg0)
		{
			if(tenant_firstName.getText().length() > 0 && tenant_lastName.getText().length() > 0
					...
					&& tenant_confirmPassword.getText().length() > 0 && tenant_phone_number.getText().length() > 0)
			{
				...
				else if(tenant_phone_number.getText().length() != 10)	
				{
				...
				else
				{
					...
					new registerTenant().execute(tenant_email.getText().toString().toLowerCase(), tenant_password.getText().toString(), 
								tenant_firstName.getText().toString(), tenant_lastName.getText().toString(), type, tenant_phone_number.getText().toString());
				}
			}
			else
			{
	...
	public void errorMsg (int title, int msg)
	{
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(Tenant_Register_Activity.this);
	...
	class registerTenant extends AsyncTask<String, String, String>
	{
		...
		{
			super.onPreExecute();
			pDialog = new ProgressDialog(Tenant_Register_Activity.this);
		...
		protected void onPostExecute(String message)
		{
			...
			if (message != null)
			{
				Toast.makeText(Tenant_Register_Activity.this, message, Toast.LENGTH_LONG).show();
			
				Intent intent = new Intent(Tenant_Register_Activity.this, TenantHomepage.class);
				intent.putExtra("EMAIL", tenant_email.getText().toString());
				intent.putExtra("PASSWORD", tenant_password.getText().toString());
				intent.putExtra("TENANT_FIRST_NAME", tenant_firstName.getText().toString());
				intent.putExtra("TENANT_LAST_NAME", tenant_lastName.getText().toString());
